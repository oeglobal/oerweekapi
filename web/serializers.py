...
class SubmissionResourceSerializer(serializers.HyperlinkedModelSerializer):
    institutionurl = serializers.CharField(
        source="institution_url", allow_blank=True, allow_null=True
    )
    ...
    directions = serializers.CharField(
        source="event_directions", allow_blank=True, allow_null=True
    )
    image_url = serializers.SerializerMethodField(read_only=True)
    post_status = serializers.CharField(read_only=True, required=False)
    post_status_friendly = serializers.SerializerMethodField(
    ...
    def validate_image(self, value):
        ...
        if width < 800 or height < 600:
            raise serializers.ValidationError(
                "Image is too small. It has to be at least 800x600px"
            )
